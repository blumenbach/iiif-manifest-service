...
// Converts SkolemIRIs to blank nodes
function convertSkolems(quads) {
    var skolemMatch = new RegExp("<(.+?)/.well-known/genid/(.+?)/(.+?)/(.+?)/(.+?)/(.+?)-(.+?)-(.+?)-(.+?)-(.+?)>", "g");
    var bnodes = quads.replace(skolemMatch, "_:b$10");
    return bnodes;
}
